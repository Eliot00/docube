...
export function makeTransformer(deps: TransformerDependencies): Effect.Effect<void, DocubeError> {
	const {
		loader,
		fileConverter,
		moduleResolver,
	...
	Effect.gen(function* () {
		...
		return {
			load: Effect.gen(function* () {
				...
				const fileNames = yield* Effect.promise(() => glob(include, globOptions));
					...
					fileNames.map((fileName) =>
						Effect.succeed({
							...
							text: Effect.tryPromise({
								try: () => readFile(fileName, { encoding: "utf-8" }).then((buf) => buf.toString()),
								catch: (error) => new DocubeError({ message: `LoadError: ${error}` }),
							}),
						}),
					),
				);
		...
		write: (file) =>
			Effect.gen(function* () {
				...
				yield* Effect.tryPromise({
					...
					catch: (error) => new DocubeError({ message: `Write error: ${error}` }),
				});
			}),
	}),
);
		...
		return {
			convert: (file) =>
				Effect.gen(function* () {
					...
					const baseName = path.basename(file._meta.fileName, path.extname(file._meta.fileName));
					let converted = yield* contentConverter.convert(file);
					if (contentTransform) {
						converted = contentTransform(converted as InternalConverterOutput, file);
					} else if (unsafePreValidation) {
					...
					return {
						...
						text: Effect.succeed(
							unsafePostContentConversion
								? unsafePostContentConversion(validated as InternalConverterOutput)
...
export const ContentValidatorLive: Layer.Layer<ContentValidator, never, Config> = Layer.effect(
	ContentValidator,
	Effect.gen(function* () {
		const config = yield* Config;
		return {
...
export const ModuleResolverLive: Layer.Layer<ModuleResolver, never, Config> = Layer.effect(
	ModuleResolver,
	Effect.gen(function* () {
		const config = yield* Config;
		return {
			resolve: (files) =>
				Effect.gen(function* () {
					...
					const imports = baseNames
						.map((baseName) => {
							return `import ${camelCase(
								baseName,
							)} from './${baseName}.json' with { type: 'json' }`;
						})
						.join("\n");
					...
					yield* Effect.promise(() => writeFile(parent, parentExport, { encoding: "utf-8" }));

					const dtsPath = path.join(baseDir, "index.d.ts");
					yield* Effect.promise(() => writeFile(dtsPath, typeStr, { encoding: "utf-8" }));
				}),
		};
	}),
);
	...
	SkipChecker.of({
		shouldSkip: (file) =>
			Effect.gen(function* () {
				...
				if (cacheHashExists) {
					const cachedHash = yield* Effect.tryPromise(() => readFile(cacheHashFilePath, "utf-8"));
