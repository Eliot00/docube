...
export function make(def: DocumentDefinition) {
    const { name, directory, includes, fields, outputBaseDir = ".docube/generated" } = def
    const ResourceLive = Layer.effect(
        Resource,
        Effect.gen(function* () {
            return {
                load: Effect.gen(function* () {
                    const fileNames = yield* Effect.promise(() => glob(includes, { cwd: directory, absolute: true }))
                    return yield* Effect.all(fileNames.map(fileName => Effect.succeed({
                        path: fileName,
        ...
        Effect.gen(function* () {
            ...
            return {
                transform: Effect.gen(function* () {
                    ...
                    yield* Effect.promise(() => fs.writeFile(path.join(outputDir, "index.mjs"), script, { encoding: 'utf-8' }))

                    const parent = path.join(outputBaseDir, "index.mjs")
                    const parentExport = `\nexport { ${exportName} } from './${pluralizedName}'`
                    yield* Effect.promise(() => fs.appendFile(parent, parentExport, { encoding: 'utf-8' }))
                    ...
                    yield* Effect.promise(() => fs.appendFile(dtsPath, defineStatement, { encoding: 'utf-8' }))

                    yield* Effect.all(files.map(file => Effect.gen(function* () {
                        ...
                        const decodeResult = Schema.decodeUnknownEither(schema)(rawOutput)
                        ...
                        yield* Effect.promise(() => fs.writeFile(path.join(outputDir, `${baseName}.json`), JSON.stringify(output, null, 2)))
                    })))
                })
            }
        })
